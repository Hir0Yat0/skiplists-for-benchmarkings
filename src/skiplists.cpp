...
void SkipList::insert(const int data) {
    int newLevel = 0; 


    // Deciding the level of inserting node on the basis of coin toss

    while (newLevel < maxNumberOfLevel && (rand() % 2) == 1) // here rand()%2 is doing the coin toss
    {
        newLevel++;
    } 
...
void SkipList::remove(const int data) {
...
bool SkipList::search(const int data) const {
...
void SkipList::display(void) const {
